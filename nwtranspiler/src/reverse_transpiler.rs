...
fn convert_python_line(line: &str) -> String {
    ...
    if let Some(caps) = Regex::new(r"^def\s+(\w+)\s*\((.*?)\)\s*(->\s*[^:]+)?:?$").unwrap().captures(&result) {
        let name = &caps[1];
        let params = &caps[2];
        let return_type = caps.get(3).map_or("", |m| m.as_str());
        return format!("function {}({}){}", name, params, return_type);
    }
    
    // Convert class
    if let Some(caps) = Regex::new(r"^class\s+(\w+).*:?$").unwrap().captures(&result) {
        let name = &caps[1];
...
fn convert_operators(code: &str) -> String {
    ...
    // Convert += 1 to ++
    result = Regex::new(r"(\w+)\s*\+=\s*1\b").unwrap().replace_all(&result, "$1++").to_string();
    
    // Convert -= 1 to --  
    result = Regex::new(r"(\w+)\s*-=\s*1\b").unwrap().replace_all(&result, "$1--").to_string();
    
    // Convert print() calls to print statements
    result = Regex::new(r"print\s*\((.*?)\)").unwrap().replace_all(&result, "print $1").to_string();
